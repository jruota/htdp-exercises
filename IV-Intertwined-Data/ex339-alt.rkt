...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex339-alt) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require htdp/dir)
...
;                (dir-find? (first lod))
;                (lod-find? (rest lod)))]))
            (ormap (lambda (x) (or (string=? (dir-name x) fod)
                                   (dir-find? x)))
                     lod))
...
(check-expect (find?.v2 TS "hang")
              #true)
(check-expect (find?.v2 TS "Images")
              #false)
