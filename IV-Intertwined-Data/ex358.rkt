...
(define (lookup-def da f)            
  (cond
    [(empty? da)
     (error NO-FUNCTION)]
    [else
     (if (symbol=? f (func-def-name (first da)))
         (first da)
...
(check-error (lookup-def.v2 DA-FGH 'i) NO-FUNCTION)

(check-expect (lookup-def.v2 DA-FGH 'f) F-FUNC)
(check-expect (lookup-def.v2 DA-FGH 'g) G-FUNC)
(check-expect (lookup-def.v2 DA-FGH 'h) H-FUNC)
