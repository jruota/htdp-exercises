...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex466-alt) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; DATA DEFINITIONS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
(check-expect (triangulate (list (list 1 2 3) (list 2 3 4)))
              (list (list 1 2 3) (list 0 -1 -2))) 
(check-expect (triangulate M2) M2)
(check-expect (triangulate M) M2)

; SOE -> SOE
; Add leading zeroes to soe.
(define (add-leading-zeroes soe)
  (cond
    [(empty? soe) '()]
    [else
     (local ((define max-length (first (sort (list-lengths soe) >)))
             ; SOE -> SOE
             ; Add leading zeroes to soe0.
             ; Wrapper function.
             (define (add-them-zeroes soe0)
               (cond
                 [(empty? soe0) '()]
                 [else
                  (cons (add-lead-zer (first soe0))
                        (add-them-zeroes (rest soe0)))]))

             ; Equation -> Equation
             ; Add leading zeroes to eq.
             ; The precise final length of the equation
             ; is derived from the longest equation in soe.
             (define (add-lead-zer eq)
               
               (cond
                 [(< (length eq) max-length)
                  (append (make-list (- max-length (length eq)) 0)
                          eq)]
                 [else eq])))
       ; – IN –
       (add-them-zeroes soe))]))

(check-expect (add-leading-zeroes '()) '())
(check-expect (add-leading-zeroes (list (list 2 2 3 10)
                                        (list 3 9 21)
                                        (list 1 2)))
              M2)

; SOE -> SOE
; Remove leading coefficients with value 0.
(define (remove-leading-zeroes soe)
  (local (; Equation -> Equation
          ; Remove leading zeroes from eq.
          ; Assumption: there is a non-zero coefficient.
          (define (rem-lead-zer eq)
            (cond
              [(empty? eq) '()]
              [else
               (if (zero? (first eq))
                   (rem-lead-zer (rest eq))
                   eq)])))
    ; – IN –
    (cond
      [(empty? soe) '()]
      [else
       (cons (rem-lead-zer (first soe))
             (remove-leading-zeroes (rest soe)))])))

(check-expect (remove-leading-zeroes '())
              '())
(check-expect (remove-leading-zeroes M2)
              (list (list 2 2 3 10)
                    (list 3 9 21)
                    (list 1 2)))

; SOE -> SOE
; Remove the leading zero in every equation in soe.
(define (remove-leading-zero soe)
  (local (; Equation -> Equation
          ; Remove a single leading zero from eq.
          ; Assumption: there is a non-zero coefficient.
          (define (away-with-zero eq)
            (if (zero? (first eq))
                (rest eq)
                eq)))
    ; – IN –
    (cond
      [(empty? soe) '()]
      [else
       (cons (away-with-zero (first soe))
             (remove-leading-zero (rest soe)))])))

(check-expect (remove-leading-zero '())
              '())
(check-expect (remove-leading-zero M2)
              (list (list 2 2 3 10)
                    (list 3 9 21)
                    (list 0 1 2)))
(check-expect (remove-leading-zero (list (list 0 0 0 0 0)
                                         (list 1 2 3 4 5)
                                         (list 1 0 1 0 1)
                                         (list 0 1 2 3 4)))
              (list (list 0 0 0 0)
...
; [List-of List] -> [List-of N]
; Return a list with the lengths of the lists in
; list l.
(define (list-lengths l)
  (cond
    [(empty? l) '()]
    [else
     (cons (length (first l))
...
(check-expect (list-lengths '()) '())
(check-expect (list-lengths (list '()
                                  (list 1)
                                  (list 1 2)
                                  (list 1 2 3)
                                  (list 1 2 3 4 5)))
