...
; Make sure that the list of obstacles lop contains
; no duplicates and is as long as demanded, i.e. has
; length n.
...
(define (render-worm new img)
  (foldr
   (lambda (ws img)
     (place-image WORM (worm-x ws) (worm-y ws) img))
   img
   new))
...
; Since "move-worm" needs to access elements of a list that come after the
; current element (see else-clause -> (worm-sed-dir (second new))) and none
; of the list abstractions provide such a funcionality, there is no suitable
; list abstraction for this function.

; END NOTE *********************************************************************

; NE-Worm -> NE-Worm
; Change the position of the worm every clock tick
; according to every segment's directional vector.
...
(define (get-worm-posns new)
  (map (lambda (x) (worm-seg-pos x)) new))
